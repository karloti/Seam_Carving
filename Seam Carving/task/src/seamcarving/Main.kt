...
class SeamCarving(inputFileName: String?, outputFileName: String?, var seamWidth: Int?, var seamHeight: Int?) {
    data class PixelEnergy(
        var pixelIn: Pair<Int, Int>,
        val pixelOut: Pair<Int, Int>,
        var pixelParent: Pair<Int, Int>? = null,
        var e: Double? = null,
        var sum: Double? = null,
    )
    ...
    fun initEnergy() { // it's fastest because haven't If expressions
    ...
    fun gradient(x1: Int, y1: Int, x2: Int, y2: Int): Int {
        val rgb = bufferedImage.getRGB(x1, y1)
        val (r1, g1, b1) = intArrayOf(rgb ushr 16 and 0xff, rgb ushr 8 and 0xff, rgb and 0xff)
        val rgb1 = bufferedImage.getRGB(x2, y2)
        val (r2, g2, b2) = intArrayOf(rgb1 ushr 16 and 0xff, rgb1 ushr 8 and 0xff, rgb1 and 0xff)
        return (r1 - r2) * (r1 - r2) + (g1 - g2) * (g1 - g2) + (b1 - b2) * (b1 - b2)
    }

    fun path(): List<Pair<Int, Int>> { // Dijkstra
        ...
        return path // it's .reversed() but not need to correct
    }
}
